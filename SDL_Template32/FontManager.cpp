...
bool FontManager::load(const std::string& file_name, const std::string& id, const int size, const int style)
{
	...
	const auto font(make_resource(TTF_OpenFont(file_name.c_str(), size)));	
	if (font != nullptr)
	{
...
bool FontManager::textToTexture(const std::string& text, const std::string& font_id, const std::string& texture_id, const SDL_Color colour)
{
	...
	const auto textSurface(make_resource(TTF_RenderText_Solid(m_fontMap[font_id].get(), text.c_str(), colour)));
	...
	{
		const auto pTexture(make_resource(SDL_CreateTextureFromSurface(Engine::Instance().GetRenderer(), textSurface.get())));
